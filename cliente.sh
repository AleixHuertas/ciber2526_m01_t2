  ...
  # IP_LOCAL=`ip -4 addr | grep 'inet ' | grep -v '127.' | awk '{print $2}' | cut -d "/" -f ip -4 addr | grep 'inet ' | grep -v '127.' | awk '{p    rint $2}' | cut -d "/" -f 11`
...
echo "RECTP $VERSION_CURRENT $IP_LOCAL" | nc $IP_SERVER -q 0 $PORT
  ...
  HASH_NAME=`echo -n "$AUDIO_FILE" | md5sum | cut -d " " -f 1`
 
  sleep 1
  echo "FILE_NAME $AUDIO_FILE $HASH_NAME" | nc $IP_SERVER -q 0 $PORT
  ...
  then
      echo "Error 2: Nombre de archivo o HASH incorrecto o mal formado"
      exit 2
  fi
